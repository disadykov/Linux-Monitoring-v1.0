...
if [ "$tmp_total" = "" ]
  then
    ./print.sh "$ram_total" "0$RAM_TOTAL GB" $1 $2 $3 $4
  else
    ./print.sh "$ram_total" "$RAM_TOTAL GB" $1 $2 $3 $4
fi
...
if [ "$tmp_used" = "" ]
  then
    ./print.sh "$ram_used" "0$RAM_USED GB" $1 $2 $3 $4
  else
    ./print.sh "$ram_used" "$RAM_USED GB" $1 $2 $3 $4
fi
...
if [ "$tmp_free" = "" ]
  then
    ./print.sh "$ram_free" "0$RAM_FREE GB" $1 $2 $3 $4
  else
    ./print.sh "$ram_free" "$RAM_FREE GB" $1 $2 $3 $4
fi
